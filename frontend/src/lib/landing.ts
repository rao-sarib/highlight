// Landing-page content model + built-in defaults (CMS fallback).
// Mirrors backend app/core/default_content.py. The admin panel edits this
// content; the landing page fetches /site/landing and falls back here.
...
export interface LandingContent {
  ...
  engines: { label: string; items: string[] };
  facts: Fact[];
  features: { eyebrow: string; title: string; subtitle: string; items: FeatureItem[] };
  steps: { eyebrow: string; title: string; subtitle: string; cta: string; items: StepItem[] };
  about: { eyebrow: string; title: string; body: string; points: string[] };
  closing: { title: string; subtitle: string; primaryCta: string; secondaryCta: string };
  faq: { eyebrow: string; title: string; items: FaqItem[] };
  footer: { tagline: string; copyright: string; email: string };
}
...
export const DEFAULT_LANDING: LandingContent = {
  ...
  hero: {
    ...
    titleTail: "it gives.",
    subtitle:
      "Highlight tells you whether ChatGPT, Perplexity and Gemini point people to you — then audits your site and writes the content that gets you cited.",
  ...
  features: {
    ...
    subtitle:
      "Find out where you stand, fix what's holding you back, and publish the content that gets picked up. No switching between five different tools.",
    items: [
      {
        icon: "Gauge",
  ...
  steps: {
    ...
    subtitle:
      "You don't fill out forms or wire up tools. Add your site and Highlight does the analysis, then hands you the work in priority order.",
  ...
  about: {
    ...
    body: "Highlight started with one question: when people ask AI assistants for recommendations, does your brand show up? We built a single workspace that measures your visibility across ChatGPT, Perplexity and Gemini, audits your site's SEO end to end, and generates the content that earns citations — so you grow in both traditional search and the new AI-driven search.",
    points: [
